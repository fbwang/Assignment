...
function setupData() {
    ...
    */
    ...
    // //chartsData.orgchart.chart = orgChart;
    // // Don't draw the orgchart, yet, because it has some conflicts with page.
    //
...
function drawCharts() {
    $('.chart_container').each(function() {
        ...
        if (chartSpec && chartSpec.chart && chartSpec.data && chartSpec.options) {
            chartSpec.chart.draw(chartSpec.data, chartSpec.options);
            if (chartSpec.animation != null) {
                setInterval(chartSpec.animation, chartSpec.animationInterval);
            }
        }
    });
}
